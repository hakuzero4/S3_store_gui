...
- 多連線設定（R2 / S3 / MinIO / 自訂），側邊欄可快速切換
- Bucket 列表 / 建立 / 刪除；同一設定下管理多桶
- 物件瀏覽、前綴目錄、搜尋、導覽列
- 上傳（按鈕 + 拖放、失敗重試、大檔分片）、下載、批次刪除、重新命名
- 跨前綴 / 跨桶批次複製與移動
- 選取項目打包 ZIP 下載；新建資料夾；物件詳情；預簽名連結
- 圖片線上預覽、文字/程式碼預覽
- 本機磁碟快取預覽/下載（系統暫存目錄，依 ETag / 修改時間校驗）
- 連線設定匯入 / 匯出
