...
- 다중 연결 프로필 (R2 / S3 / MinIO / 커스텀), 사이드바에서 빠른 전환
- 버킷 목록 / 생성 / 삭제, 프로필당 여러 버킷
- 객체 탐색(접두사), 검색, 브레드크럼
- 업로드(버튼 + 드래그, 재시도, 대용량 멀티파트), 다운로드, 일괄 삭제, 이름 변경
- 접두사/버킷 간 일괄 복사·이동
- 선택 항목 ZIP 다운로드, 폴더 생성, 객체 상세, presigned URL
- 이미지 미리보기, 텍스트/코드 미리보기
- 미리보기/다운로드 로컬 디스크 캐시(임시 디렉터리, ETag / 수정 시간 검증)
- 프로필 가져오기 / 내보내기
- **UI 다국어:** English(기본), 简体中文, 繁體中文, 日本語, 한국어
...
| `Release` | tag `v*` (예: `v0.2.0`) | 다중 플랫폼 바이너리 + GitHub Release |
...
예: `s3store_v0.2.0_windows_amd64.exe`, `linux_amd64/arm64`, `darwin_amd64/arm64`, `checksums.txt`
...
- [x] 대용량 파일 멀티파트 업로드
- [x] 접두사/버킷 간 복사·이동
- [x] ZIP 다운로드, 텍스트 미리보기, 로컬 객체 캐시
- [ ] 다크 모드
- [ ] Docker용 선택적 Basic Auth
